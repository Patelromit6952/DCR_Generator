...
import { FaChevronDown, FaChevronUp, FaCalculator, FaTrash, FaSave, FaPrint, FaPlus, FaDownload, FaFileAlt } from 'react-icons/fa';
...
export const DisplaySedB = () => {
  ...
  const [activeCell, setActiveCell] = useState({ scheduleIndex: null, itemIndex: null });
  const [showQtyCalc, setShowQtyCalc] = useState({});
  ...
  const totalGrand = Data.reduce((acc, schedule) => {
    const total = schedule.item_of_works.reduce((sum, item) => sum + item.amount, 0);
    return acc + total;
  }, 0);

  const gstAmount = totalGrand * 0.18;
  const finalTotal = totalGrand + gstAmount;
  ...
  const handleRemoveRow = (scheduleIndex, itemIndex) => {
    const ans = confirm("Are You Really Want To Remove This Row ?");
    if (ans) {
      const updated = [...Data];
  ...
  const toggleSchedule = (scheduleName) => {
    const updatedSet = new Set(expandedSchedules);
    if (updatedSet.has(scheduleName)) {
      updatedSet.delete(scheduleName);
    } else {
      updatedSet.add(scheduleName);
    }
    setExpandedSchedules(updatedSet);
  };
  const toggleQtyCalc = (scheduleIndex, itemIndex) => {
    ...
    if (!showQtyCalc[key]) {
      const updated = [...Data];
      if (!updated[scheduleIndex].item_of_works[itemIndex].calcRows.length) {
        updated[scheduleIndex].item_of_works[itemIndex].calcRows.push({
          length: 0,
          width: 0,
          height: 1,
          description: ''
  ...
  const handleCalcRowChange = (scheduleIndex, itemIndex, rowIndex, field, value) => {
    const updated = [...Data];
    updated[scheduleIndex].item_of_works[itemIndex].calcRows[rowIndex][field] =
      field === 'description' ? value : (parseFloat(value) || 0);
    setData(updated);
  };
  const addCalcRow = (scheduleIndex, itemIndex) => {
    const updated = [...Data];
    updated[scheduleIndex].item_of_works[itemIndex].calcRows.push({
      length: 0,
      width: 0,
  ...
  const deleteCalcRow = (scheduleIndex, itemIndex, rowIndex) => {
    const updated = [...Data];
    updated[scheduleIndex].item_of_works[itemIndex].calcRows.splice(rowIndex, 1);
    setData(updated);
  };
  const saveCalculation = (scheduleIndex, itemIndex) => {
    const updated = [...Data];
    const rows = updated[scheduleIndex].item_of_works[itemIndex].calcRows;
    const totalQty = rows.reduce((sum, row) => sum + row.length * row.width * (row.height || 1), 0);
    updated[scheduleIndex].item_of_works[itemIndex].qty = totalQty;
    updated[scheduleIndex].item_of_works[itemIndex].amount = totalQty * updated[scheduleIndex].item_of_works[itemIndex].rate;
    setData(updated);
        ...
        <div class="summary">
            <table class="summary-table">
                <tr>
                    ...
                    <td class="amount">₹${totalGrand.toFixed(2)}</td>
                </tr>
                <tr>
...
<body>
  ...
  const allowedSchedules = ["B1", "B2", "B3", "B4", "B5"];
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-2xl font-bold text-gray-800">SED-B Quatation</h1>
                    ...
                    <div key={draft.id} className="bg-white p-3 rounded border flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">
                          Client: {draft.clientDetails.name || 'N/A'} | Total: ₹{draft.finalTotal?.toFixed(2) || '0.00'}| Saved: {draft.savedAt}
                        </div>
                      </div>
        ...
        {/* Schedule Blocks */}
        {Data
          .filter(schedule => allowedSchedules.includes(schedule.schedule))
          .map((scheduleBlock, scheduleIndex) => {
            const scheduleTotal = scheduleBlock.item_of_works.reduce((sum, item) => sum + item.amount, 0);
            return (
              <div key={scheduleBlock._id} className="bg-white rounded-lg shadow-lg overflow-hidden mb-6">
                <div
                  ...
                  onClick={() => toggleSchedule(scheduleBlock.schedule)}
                >
                  <h2 className="text-sm font-semibold flex items-center gap-3">
                    <span className="bg-blue-600 px-3 py-1 rounded-full text-sm">
                      Schedule {scheduleBlock.schedule}
                    </span>
                    {scheduleBlock.description || 'Work Items'}
                  </h2>
                  <span className="text-xl">
                    {expandedSchedules.has(scheduleBlock.schedule) ? <FaChevronUp /> : <FaChevronDown />}
                  </span>
                </div>

                {expandedSchedules.has(scheduleBlock.schedule) && (
                  <div className="p-6">
                    {scheduleBlock.item_of_works?.map((item, itemIndex) => {
                      const calcKey = `${scheduleIndex}-${itemIndex}`;
                      const totalRowQty = item.calcRows?.reduce(
                        (sum, row) => sum + row.length * row.width * (row.height || 1),
                        0
                      ) || 0;
                              ...
                              <div>
                                ...
                                <div className="flex gap-2">
                                  <input
                                    ...
                                    className="flex-1 px-1 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                    placeholder="0.00"
                                  />
                                  <button
                                  ...
                                  <table className="w-full bg-white border border-gray-200 rounded-lg">
                                    <thead className="bg-gray-100">
                                      <tr>
                                        ...
                                        <th className="py-2 px-3 border">Qty ({item.unit || 'Unit'})</th>
                                        ...
                                        <tr key={rowIndex} className="hover:bg-gray-50">
                                          <td className="py-2 px-3 border">
                                            <input
                                              ...
                                              onChange={(e) =>
                                                handleCalcRowChange(scheduleIndex, itemIndex, rowIndex, 'description', e.target.value)
                                              }
                                          ...
                                          <td className="py-2 px-3 border">
                                            <input
                                              ...
                                              value={row.length || 0}
                                              onChange={(e) =>
                                                handleCalcRowChange(scheduleIndex, itemIndex, rowIndex, 'length', e.target.value)
                                              }
                                          ...
                                          <td className="py-2 px-3 border">
                                            <input
                                              ...
                                              value={row.width || 0}
                                              onChange={(e) =>
                                                handleCalcRowChange(scheduleIndex, itemIndex, rowIndex, 'width', e.target.value)
                                              }
                                              className="w-20 px-2 py-1 border rounded text-center"
                                              placeholder="0"
                                            />
                                          ...
                                          <td className="py-2 px-3 border">
                                            <input
                                              ...
                                              value={row.height || 1}
                                              onChange={(e) =>
                                                handleCalcRowChange(scheduleIndex, itemIndex, rowIndex, 'height', e.target.value)
                                              }
                                              className="w-20 px-2 py-1 border rounded text-center"
                                              placeholder="1"
                                            />
                                          ...
                                          <td className="py-2 px-3 border text-center font-semibold text-blue-600">
                                            {((row.length || 0) * (row.width || 0) * (row.height || 1)).toFixed(2)}
                                          </td>
                    ...
                    <div className="mt-6 bg-gradient-to-r from-gray-100 to-gray-200 rounded-lg p-4">
                      <h3 className="text-lg font-bold text-gray-800 mb-3">Schedule Summary</h3>
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold">₹{(scheduleTotal * 0.18).toFixed(2)}</span>
                        </div>
                        <hr className="border-gray-300" />
                        <div className="flex justify-between items-center">
                          <span className="text-lg font-bold text-gray-800">Schedule Total:</span>
                          <span className="text-lg font-bold text-green-600">
                            ₹{(scheduleTotal * 1.18).toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-gray-800 to-gray-700 text-white px-6 py-4">
            <h2 className="text-xl font-semibold">Overall Project Summary</h2>
          </div>
          ...
          <div className="p-6">
            <div className="overflow-x-auto">
              <table className="w-full border border-gray-200 rounded-lg">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="py-3 px-4 border text-left font-semibold">Schedule</th>
                    <th className="py-3 px-4 border text-left font-semibold">Description</th>
                    <th className="py-3 px-4 border text-right font-semibold">Grand Total</th>
                  </tr>
                </thead>
                <tbody>
                  {Data.map((schedule, index) => {
                    ...
                    return (
                      <tr key={index} className="hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 border text-gray-700">{schedule.description || '-'}</td>
            ...
            {/* Final Total */}
            <div className="mt-6 bg-gradient-to-r from-green-100 to-green-200 rounded-lg p-6">
              <div className="flex justify-between items-center">
                <span className="text-2xl font-bold text-gray-800">Project Total Amount:</span>
                <span className="text-3xl font-bold text-green-600">₹ {totalGrand.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Modal outside max-w-7xl container */}
      {modalOpen && (
